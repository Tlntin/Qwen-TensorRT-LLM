...
SmoothQuantGemmPlugin::SmoothQuantGemmPlugin(bool perChannelScaling, bool perTokenScaling, nvinfer1::DataType type)
{
...
void SmoothQuantGemmPlugin::init(bool perChannelScaling, bool perTokenScaling, nvinfer1::DataType type)
{
    mType = type;
...
{
    auto* plugin
        = new SmoothQuantGemmPlugin(m_quantOption.hasPerChannelScaling(), m_quantOption.hasPerTokenScaling(), mType);
...
IPluginV2* SmoothQuantGemmPluginCreator::createPlugin(const char* name, const PluginFieldCollection* fc) noexcept
{
    ...
    try
    {
        auto* obj = new SmoothQuantGemmPlugin(perChannelScaling, perTokenScaling, type);
