...
GptSessionWithDecoderBatch::GptSessionWithDecoderBatch(GptModelConfig const& modelConfig,
    WorldConfig const& worldConfig, void const* engineBuffer, std::size_t engineSize, LoggerPtr logger)
...
void GptSessionWithDecoderBatch::createContexts()
{
...
void GptSessionWithDecoderBatch::RuntimeBuffers::create(TllmRuntime& runtime, GptModelConfig const& modelConfig)
{
...
void GptSessionWithDecoderBatch::RuntimeBuffers::reshape(
...
void GptSessionWithDecoderBatch::RuntimeBuffers::tile(
    BufferManager& manager, GenerationConfig const& generationConfig, bool usePlugin)
{
...
void GptSessionWithDecoderBatch::setup(SizeType const batchSize, SizeType const maxInputLength,
    SizeType const maxNewTokens, SizeType const endId, SizeType const padId, SamplingConfig const& samplingConfig)
{
    ...
    createDecoder(endId, padId);
    mDecoder->setup(batchSize, samplingConfig.beamWidth, maxInputLength + maxNewTokens);

    mBuffers.reshape(mGenerationConfig, mModelConfig, mWorldConfig.getSize());
}

void GptSessionWithDecoderBatch::createDecoder(SizeType const endId, SizeType const padId)
{
    mDecoder = std::make_shared<GptDecoderBatch>(mModelConfig.getVocabSize(),
        mModelConfig.getVocabSizePadded(mWorldConfig.getSize()), endId, padId, mRuntime->getStreamPtr());
}
...
void GptSessionWithDecoderBatch::getContextBuffers(
...
void GptSessionWithDecoderBatch::getNextStepBuffers(TensorMap& inputBuffers, TensorMap& outputBuffers,
    SizeType const step, TensorPtr const& inputIds, TensorPtr const& inputLengths) const
{
    inputBuffers.clear();
    outputBuffers.clear();
...
void GptSessionWithDecoderBatch::setRawPointers(
...
void GptSessionWithDecoderBatch::prepareContextStep(GenerationInput const& inputs)
{
...
GptSessionWithDecoderBatch::TensorPtr GptSessionWithDecoderBatch::prepareNextStep(
    SizeType const step, bool const inputPacked, TensorPtr const& outputIds)
{
    auto& manager = mRuntime->getBufferManager();
    auto& stream = mRuntime->getStream();

    TensorPtr nextInputIds = ITensor::view(outputIds);
    if (inputPacked)
    {
...
void GptSessionWithDecoderBatch::copyInputToOutput(TensorPtr const& outputIds, TensorPtr const& inputIds,
    TensorPtr const& inputLengths, SizeType const padId, SizeType const maxInputLength, bool const inputPacked) const
{
    auto& stream = mRuntime->getStream();
...
void GptSessionWithDecoderBatch::checkInputShape(
    TensorPtr const& inputIds, TensorPtr const& inputLengths, bool const inputPacked) const
{
...
void GptSessionWithDecoderBatch::generate(
    GenerationOutput& outputs, GenerationInput const& inputs, std::vector<SamplingConfig> const& samplingConfigs)
{
    ...
    // input for decoding
    decoder_batch::Input decodingInput{mBuffers.logits};

    // output for decoding
    decoder_batch::Output decodingOutput{};
    ...
    { // split batch into single requests
        ...
        auto inputLengthsHost = manager.copyFrom(*inputs.lengths, MemoryType::kCPU);
        auto inputLengthsPtr = bufferCast<SizeType>(*inputLengthsHost);
        auto inputOffset = 0;
        for (auto b = 0; b < batchSize; ++b)
        {
            auto const inputLength = inputLengthsPtr[b];
            ...
            {
                inputView = ITensor::slice(inputs.ids, b, 1);
                inputView->reshape(inputShape);
            }
            auto request = decoder_batch::Request{inputView, maxNewTokens};
            ...
            mDecoder->newRequest(b, request, samplingConfigs[b]);
        }
    }

    if (beamWidth > 1)
    {
        mBuffers.parentIds->reshape(ITensor::makeShape({maxSeqLength, batchSize, beamWidth}));
    ...
    for (SizeType step = 0; step < maxNewTokens; ++step)
    {
        ...
        if (step < maxNewTokens - 1)
        {
            ...
            auto nextInputIds = prepareNextStep(step, inputs.packed, mDecoder->getNewTokens());
            getNextStepBuffers(
        ...
        mDecoder->forward(decodingOutput, decodingInput);

        auto finished = mDecoder->getFinished();
        auto shouldStop = std::any_of(finished.begin(), finished.end(), [](auto const& x) { return x; });
        ...
        if (shouldStop)
        {
            ...
            auto decoderOutputIdsView = ITensor::slice(mDecoder->getOutputIds(), 0, batchSize);
            auto const& decoderOutputIdsShape = decoderOutputIdsView->getShape();
            decoderOutputIdsView->reshape(ITensor::makeShape(
                {decoderOutputIdsShape.d[0] * decoderOutputIdsShape.d[1], decoderOutputIdsShape.d[2]}));

            auto const outputIdsOriginalShape = outputs.ids->getShape();
            outputs.ids->reshape(ITensor::makeShape(
                {outputIdsOriginalShape.d[0], outputIdsOriginalShape.d[1] * outputIdsOriginalShape.d[2]}));
            kernels::invokeTransposeWithOutputOffset(*outputs.ids, *decoderOutputIdsView, 0, manager.getStream());
            outputs.ids->reshape(ITensor::makeShape(
                {outputIdsOriginalShape.d[0], outputIdsOriginalShape.d[1], outputIdsOriginalShape.d[2]}));
    ...
    if (beamWidth > 1)
    {
        // Reshape and transpose parentIds
        auto decoderParentIdsView = ITensor::slice(mDecoder->getParentIds(), 0, batchSize);
        auto const& decoderParentIdsShape = decoderParentIdsView->getShape();
        decoderParentIdsView->reshape(
            ITensor::makeShape({decoderParentIdsShape.d[0] * decoderParentIdsShape.d[1], decoderParentIdsShape.d[2]}));

        auto const parentIdsOriginalShape = mBuffers.parentIds->getShape();
        mBuffers.parentIds->reshape(ITensor::makeShape(
            {parentIdsOriginalShape.d[0], parentIdsOriginalShape.d[1] * parentIdsOriginalShape.d[2]}));
        kernels::invokeTranspose(*mBuffers.parentIds, *decoderParentIdsView, manager.getStream());
        mBuffers.parentIds->reshape(ITensor::makeShape(
            {parentIdsOriginalShape.d[0], parentIdsOriginalShape.d[1], parentIdsOriginalShape.d[2]}));
        ...
        gatherTree(outputs.ids, mDecoder->getOutputLengths(), endIds, finalSeqLength);
    }
    else
    {
        auto tmpOutputIds = manager.copyFrom(*outputs.ids, MemoryType::kGPU);
        tmpOutputIds->reshape(ITensor::makeShape({finalSeqLength, batchSize * beamWidth}));

        outputs.ids->reshape(ITensor::makeShape({batchSize, beamWidth, finalSeqLength}));
        kernels::invokeTranspose(*outputs.ids, *tmpOutputIds, stream);
    }
}

// this should be similar to gatherTree in cpp/tensorrt_llm/thop/gatherTreeOp.cpp
void GptSessionWithDecoderBatch::gatherTree(
    TensorPtr& outputIds, TensorPtr const& sequenceLength, TensorPtr const& endIds, SizeType const finalSeqLength) const
{
    ...
    param.has_padding = !mModelConfig.useInflightBatchingGptAttentionPlugin();
...
std::vector<uint8_t> tensorrt_llm::runtime::GptSessionWithDecoderBatch::loadEngine(std::string const& enginePath)
{
...
nvinfer1::ILogger& tensorrt_llm::runtime::GptSessionWithDecoderBatch::getLogger() const
{
    return *mLogger;
}

BufferManager& tensorrt_llm::runtime::GptSessionWithDecoderBatch::getBufferManager() const
{
    return mRuntime->getBufferManager();
}

int GptSessionWithDecoderBatch::initDevice(WorldConfig const& worldConfig)
{
...
void GptSessionWithDecoderBatch::RuntimeBuffers::clear()
{
    logits = nullptr;
