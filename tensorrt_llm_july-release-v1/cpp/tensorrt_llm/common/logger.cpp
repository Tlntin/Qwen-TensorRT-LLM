...
void Logger::log(TllmException const& ex, Logger::Level const level)
{
    log(level, "%s: %s\n%s", TllmException::demangle(typeid(ex).name()).c_str(), ex.what(), ex.getTrace().c_str());
}

void Logger::log(std::exception const& ex, Logger::Level level)
{
    if (typeid(ex) == typeid(TllmException))
    {
        log(dynamic_cast<TllmException const&>(ex), level);
    }
    else
    {
