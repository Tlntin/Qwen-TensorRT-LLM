...
public:
    ...
    void setup(const int b, const int s, const int total_seqlen)
    {
        const float scale_bmm1 = 1.f / (sqrtf(mHeadSize) * mQScaling);
        ...
        //   qkv_layout = [b, s, [q_hd, k_d, v_d]]
        //   qkv_stride = (h+2)d * bytes_per_elt
        // Otherwise:
        //   qkv_layout = [b, s, 3, h, d]
        //   qkv_stride = 3hd * bytes_per_elt
        if (params.multi_query_attention)
        {
            params.qkv_stride_in_bytes = (mNumHeads + 2) * mHeadSize * sizeof(half);
        }
        else
        {
            params.qkv_stride_in_bytes = 3 * mNumHeads * mHeadSize * sizeof(half);
        }
        params.o_stride_in_bytes = mNumHeads * mHeadSize * sizeof(half);

        // Hooper: fallback to original fmha_v2 when head_size <= 64 and seq_len <= 256
        const bool hopper_kernel = (sm == kSM_90);
        const bool ampere_ada_kernel = (sm == kSM_80 || sm == kSM_86 || sm == kSM_89);
        if (hopper_kernel && params.d <= 64 && params.s <= 256)
        {
        ...
        { // always use flash attention kernels for Ampere/Ada
            ...
            if (ampere_ada_kernel)
            {
                ...
                launch_params.granular_tiling = launch_params.flash_attention && (params.s > 64);
            }
        }

        // when flash attention is enabled on Hopper, we need to set the tma descriptors
        if (hopper_kernel && launch_params.flash_attention)
        {
        ...
        const bool force_fp32_acc, const bool is_s_padded, const bool causal_mask, const bool multi_query_attention)
    {
        ...
        launch_params.causal_mask = causal_mask;
        params.multi_query_attention = multi_query_attention;
...
void FusedMHARunnerV2::setup(const int b, const int s, const int total_seqlen)
{
    pimpl->setup(b, s, total_seqlen);
}
...
void FusedMHARunnerV2::setup_flags(
    const bool force_fp32_acc, const bool is_s_padded, const bool causal_mask, const bool multi_query_attention)
{
    pimpl->setup_flags(force_fp32_acc, is_s_padded, causal_mask, multi_query_attention);
}
...
int FusedMHARunnerV2::getSFromMaxSeqLen(const int max_seq_len)
{
