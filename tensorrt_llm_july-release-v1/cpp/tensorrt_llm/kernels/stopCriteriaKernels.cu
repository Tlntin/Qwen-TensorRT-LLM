...
__global__ void stop_words_criterion(const int* output_ids, const int* parent_ids, const int* stop_words,
    bool* finished, size_t id_offset, size_t stop_words_len, int batch_size, int beam_width, int step)
{
...
void invokeStopWordsCriterion(const int* output_ids, const int* parent_ids, const int* stop_words, bool* finished,
    size_t id_offset, size_t stop_words_len, int batch_size, int beam_width, int step, cudaStream_t stream)
{
...
__global__ void length_criterion(
    bool* finished, int* finished_sum, const uint32_t* sequence_limit_length, int batch_size, int beam_width, int step)
{
...
void invokeLengthCriterion(bool* finished, int* finished_sum, const uint32_t* sequence_limit_length, int batch_size,
    int beam_width, int step, cudaStream_t stream)
{
    ...
    length_criterion<<<grid, block, 0, stream>>>(
        finished, finished_sum, sequence_limit_length, batch_size, beam_width, step);
    sync_check_cuda_error();
}
