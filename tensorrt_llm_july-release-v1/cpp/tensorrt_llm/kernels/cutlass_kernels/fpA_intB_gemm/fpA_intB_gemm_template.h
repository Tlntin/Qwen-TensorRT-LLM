...
void generic_mixed_gemm_kernelLauncher(const T* A, const WeightType* B, const T* weight_scales, const T* biases, T* C,
    int m, int n, int k, tkc::CutlassGemmConfig gemm_config, char* workspace, size_t workspace_bytes,
    cudaStream_t stream, int* occupancy = nullptr)
{
    ...
    //         "[TensorRT-LLm Error][fpA_intB] FasterTransformer was built was mixed gemm support off. Please rebuild
    //         with cmake option "
...
struct dispatch_stages
{
    static void dispatch(const T* A, const WeightType* B, const T* weight_scales, const T* biases, T* C, int m, int n,
        int k, tkc::CutlassGemmConfig gemm_config, char* workspace, size_t workspace_bytes, cudaStream_t stream,
        int* occupancy = nullptr)
    {
        ...
        throw std::runtime_error("[TensorRT-LLm Error][dispatch_stages::dispatch] " + err_msg);
    }
};
...
void dispatch_gemm_to_cutlass(const T* A, const WeightType* B, const T* weight_scales, const T* biases, T* C, int m,
    int n, int k, char* workspace, size_t workspace_bytes, tkc::CutlassGemmConfig gemm_config, cudaStream_t stream,
    int* occupancy = nullptr)
{
...
void CutlassFpAIntBGemmRunner<T, WeightType>::dispatch_to_arch<EpilogueTag>(const T* A, const WeightType* B,
    const T* weight_scales, const T* biases, T* C, int m, int n, int k, tkc::CutlassGemmConfig gemm_config,
...
void CutlassFpAIntBGemmRunner<T, WeightType>::run_gemm<EpilogueTag>(const T* A, const WeightType* B,
    const T* weight_scales, const T* biases, T* C, int m, int n, int k, char* workspace_ptr,
    const size_t workspace_bytes, cudaStream_t stream)
{
    ...
    {
        dispatch_to_arch<EpilogueTag>(A, B, weight_scales, biases, C, m, n, k, candidate_configs[ii], workspace_ptr,
...
template <typename T, typename WeightType>
void CutlassFpAIntBGemmRunner<T, WeightType>::gemm(const void* A, const void* B, const void* weight_scales, void* C,
    int m, int n, int k, char* workspace_ptr, const size_t workspace_bytes, cudaStream_t stream)
{
    TLLM_LOG_DEBUG(__PRETTY_FUNCTION__);
    run_gemm<tkc::EpilogueOpNoBias>((const T*) A, (const WeightType*) B, (const T*) weight_scales, nullptr, (T*) C, m,
        n, k, workspace_ptr, workspace_bytes, stream);
}
