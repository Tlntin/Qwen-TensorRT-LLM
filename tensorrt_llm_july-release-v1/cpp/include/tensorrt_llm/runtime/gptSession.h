...
public:
    ...
    GptSession(GptModelConfig const& modelConfig, WorldConfig const& worldConfig, std::string const& engineFile,
        ...
        : GptSession(modelConfig, worldConfig, loadEngine(engineFile), logger)
    {
    }
    ...
    void setup(
        SizeType batchSize, SizeType maxInputLength, SizeType maxNewTokens, SamplingConfig const& samplingConfig);

    void generate(GenerationOutput& outputs, GenerationInput const& inputs);
...
private:
    ...
    std::unique_ptr<IGptDecoder> createDecoder();
...
private:
    ...
    std::shared_ptr<IGptDecoder> mDecoder;
