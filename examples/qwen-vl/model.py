...
from tensorrt_llm.models.gpt.model import GPTEmbedding
...
class QWenModel_VL(Module):
    ...
    def forward(
        ...
        if self.mapping.is_first_pp_rank():
            ...
            # hidden_states = self.vocab_embedding(input_ids)#input_ids:(1,-1)
        else:
            hidden_states = recv(hidden_states, self.mapping.prev_pp_rank())
        self.register_network_output(f"embd", hidden_states)
        #hidden_states = (-1, -1, 4096), 第一个-1是batch_size,第二个是input_len
        # hidden_states[bos_pos_0][bos_pos_1 + 1 : eos_pos_1] = visual_embedding[bos_pos_0]#visual_embedding.shape=torch.Size([1, 256, 4096])...hidden_states[i][a + 1 : b].shape=torch.Size([256, 4096])
        # slice(hidden_states,[bos_pos_0,bos_pos_1 + 1,0],[eos_pos_1-bos_pos_1,hidden_size])
        # hidden_states = qwen_visual_fusion(hidden_states,visual_embedding,img_pos)
        # hidden_states = replace_slice(hidden_states,visual_embedding,start)
...
class QWenForCausalLM_VL(QWenModel_VL, GenerationMixin):
    ...
    def forward(
        ...
        if use_cache and default_net().plugin_config.paged_kv_cache == False:
            for i, present in zip(self.mapping.pp_layers(self.num_layers), presents):
    ...
    def prepare_inputs(
        ...
        return (model_inputs['input_ids'], 
                model_inputs['position_ids'], True,
