...
class QWenInfer(object):
    def __init__(
        ...
        self.global_max_output_len = global_max_output_length
        self.decoder = None
        self.tokenizer = None
        self.config = None
    ...
    def make_context(
        ...
        if chat_format == "chatml":
            ...
            for turn_query, turn_response in reversed(history):
                query_text, query_tokens_part = _tokenize_str("user",
                                                              turn_query)
                ...
                current_context_size = (
                        len(system_tokens) + len(next_context_tokens) + len(
                    context_tokens)
                )
    ...
    def generate_for_qwen_vl(
        ...
        # input_ids = None
        # input_lengths = None
        input_ids = torch.as_tensor(input_tokens, device="cuda",
                                    dtype=torch.int32)
    ...
    def qwen_infer(
        ...
        prompt_table, tasks, task_vocab_size = self.ptuning_setup(
            ...
            input_ids,
            input_lengths,
            self.model_config.remove_input_padding
        )  # [prompt_table, tasks, task_vocab_size]

        output_ids = self.generate_for_qwen_vl(
            ...
            if self.output_csv is None and self.output_npy is None:
                for b in range(input_lengths.size(0)):
                    ...
                    if self.num_beams <= 1:
                        ...
                        try:
                            effective_output_token = effective_output_token + outputs.index(
                                151643)
                        except:
    ...
    def qwen_infer_stream(
        ...
        runtime_rank = tensorrt_llm.mpi_rank()
        for output_dict in self.generate_for_qwen_vl(
                input_ids,
                max_new_tokens,
                prompt_table,
                tasks,
            ...
            if runtime_rank == 0:
                ...
                for i in range(output_ids.size(0)):
                    temp_ids = output_ids[i, 0, input_lengths.item(): sequence_lengths[i][0].item()]
                    pure_ids = []
                    for j in range(len(temp_ids)):
                        if temp_ids[j] in [self.tokenizer.im_start_id,
                    ...
                    if pure_ids.size(0) == 0:
                        continue
                    temp_text = self.tokenizer.decode(pure_ids, skip_special_tokens=False)
                    # check code is error
                    if b"\xef\xbf\xbd" in temp_text.encode():
                        continue
                    input_lengths += pure_ids.size(0)
                    output_texts.append(temp_text)
                if len(output_texts) > 0:
                    yield output_texts[0]
...
class Vit:
    def __init__(self, engine_dir, log_level):
